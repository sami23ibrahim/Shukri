...
function renderWithColors(line) {
  const parts = [];
  let remaining = line;
  ...
  for (const { word, color } of all) {
    ...
    parts.push(<span key={parts.length} className={color === "red" ? "text-red-500" : "text-green-500"}>{word}</span>);
...
function ScrolledLinesV2({ lines = defaultLines }) {
  ...
  const sectionHeight = `${100 + (lines.length - 1) * 35}vh`;
